        ...
        for y in range(point.y - 1, point.y + 2):
            for z in range(point.z - 1, point.z + 2):
                for w in range(point.w - 1, point.w + 2):
                    if (point.x, point.y, point.z, point.w) == (x, y, z, w):
                        continue
                    if Point(x, y, z, w) in pocket:
                        active_neighbors += 1
    if active:
...
def simulate(pocket: Set[Point]) -> Set[Point]:
    ...
    max_w = max([point.w for point in pocket])
    min_w = min([point.w for point in pocket])
